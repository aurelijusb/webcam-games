...
//        cvRectangle(canvas, cvPoint(x - sizeX / 2, y - sizeY / 2), cvPoint(x + sizeX / 2, y + sizeY / 2), color1, 2);
        ...
        for (int i = 0; i < 360; i += 25 + rand() % 40) {
            double aX = cos((float) (i+rand() % 13) / 180 * 3.14) * (rand() % sizeX / 2);
            double aY = sin((float) (i+rand() % 14) / 180 * 3.14) * (rand() % sizeY / 2);
            CvSize size = cvSize(round(angeX) + sizeX, round(angeY) + sizeY);
            CvSize size2 = cvSize(size.width * 4 / 10 + rand() % 12, size.height * 8 / 10 + rand() % 4);
            CvScalar color1 = CV_RGB(rand() % 128 + 128, 255 - rand() % 60, 255);
            cvEllipse(canvas, point, size, 0, i, i + rand() % 5, color1,1);
            cvEllipse(canvas, point, size2, 0, i + rand() % 2, i + 5 + rand() % 14, color1);
...
//    cvCircle(canvas, cvPoint(x, y), sizeX, CV_RGB(255, 255, 0), 1);
}
...
void Bubble::boom() {
    if (state == normal) {
        state = exploding;
//        FILE *stream;
//        stream = popen("mplayer /home/PAPILDOMI/VIDEOCAMP/openCV1/pokst.ogg", "r");
//        while ( fgetc(stream) );
//        pclose(stream);   
    }
}
